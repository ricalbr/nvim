...
vim.opt.makeprg =
'g++ -std=c++17 -Wshadow -Wall -g -I include % -fsanitize=address -fsanitize=undefined -D_GLIBCXX_DEBUG -o %:r'
vim.keymap.set('n', '<F6>', '<cmd> make<CR>', opts)
vim.keymap.set('i', '<F6>', '<Esc> <cmd>make <CR>', opts)

-- compile with -O2 flag (optimized)
vim.keymap.set('n', '<F7>', '<cmd>w <bar> !g++ -std=c++17 -Wshadow -Wall -O2 -Wno-unused-result -I include % -o %:r <CR>',
    opts)
vim.keymap.set('i', '<F7>',
    '<Esc><cmd>w <bar> !g++ -std=c++17 -Wshadow -Wall -O2 -Wno-unused-result -I include % -o %:r <CR>', opts)
